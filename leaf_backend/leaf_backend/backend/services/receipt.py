...
class DefaultStrategy(Strategy):
    def get_blocks(self, lines):
        ...
        column = 0
        begin_row = 0
        while len(lines) - 1 > row:

            # Check end_col
            col_pos = len(lines[row])
            column = col_pos if column < col_pos else column

            # If the length of repeated characters is not greater than 1 + '\n'
            ...
            condition = lines[row + 1] == '\n' or len("".join(set(lines[row + 1])).strip()) <= 2
            while condition and len(lines) - 1 > row:
                row += 1
                # START Check end_col
                col_pos = len(lines[row])
                column = col_pos if column < col_pos else column
